 ...
 * 获取地区列表
 */
function get_region_list1() {
	// return M('region')->cache(true)->getField('id as value,name as text');
	// $res = M('region')
	// 	->field('id as value, name as text,parent_id')
	// 	->where('parent_id', 0)
	// 	->cache(true)
	// 	->select();
	// if ($res) {
	// 	foreach ($res as $k => $v) {
	// 		$res[$k]['children'] = M('region')
	// 			->field('id as value, name as text,parent_id')
	// 			->where('parent_id', $v['value'])
	// 			->cache(true)
	// 			->select();
	// 		if ($res[$k]['children']) {
	// 			foreach ($res[$k]['children'] as $kk => $vv) {
	// 				$res[$k]['children'][$kk]['children'] = M('region')
	// 					->field('id as value, name as text,parent_id')
	// 					->where('parent_id', $vv['value'])
	// 					->cache(true)
	// 					->select();
	// 			}
	// 		}
	...
	// 		if ($arr[$key]['id'] == $res[$k]['parent_id']) {
	// 			$arr[$key]['children'][] = $res[$k];
	// 			unset($res[$k]);
	// 		}
	// 		// if ($arr[$key]['children']) {
	// 		// 	foreach ($arr[$key]['children'] as $kk => $vv) {
	// 		// 		if ($arr[$key]['children'][$kk]['id'] == $res[$k]['parent_id']) {
	// 		// 			$arr[$key]['children'][$kk]['children'][] = $res[$k];
	// 		// 			unset($res[$k]);
	// 		// 		}
	// 		// 	}
	// 		// }
 ...
 * 获取用户地址列表
 */
function get_user_addres_list($user_id) {
	$lists = M('user_address')
		->field('province,city,district,twon')
	...
	$res = M('region')->where('id', 'in', $str)->cache(true)->column('id,name');
	return $res;
}
/*
 * 获取指定地址信息
