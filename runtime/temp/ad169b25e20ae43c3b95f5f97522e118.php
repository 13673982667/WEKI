<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:41:"./template/mobile/rainbow/cart\index.html";i:1530846810;s:63:"D:\phpStudy\WWW\shop\template\mobile\rainbow\public\header.html";i:1530846810;s:67:"D:\phpStudy\WWW\shop\template\mobile\rainbow\public\header_nav.html";i:1530846810;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8" />
                            ...
                            <div class="sc_list">
                                ...
                                <div class="shopimg fl">
                                    <a href="<?php echo U('Mobile/Goods/goodsInfo',array('id'=>$cart[goods_id])); ?>">
                                        <!--商品图片-->
                                        <img src="<?php echo goods_thum_images($cart['goods_id'],400,400); ?>">
                                    </a>
                                </div>
                ...
                <!--购物车没有商品-e-->
                    ...
                    <div class="Set-meal-wrap ">
                        <div class="orderlistshpop-titles p">
                            ...
                            <span class="fr" style="line-height: 2.0266rem;color: #f23030;font-size: .61rem;margin-top: 0.32rem;display: block;" >共优惠：￥<em id="countDiscount_<?php echo $cart['id']; ?>"><?php echo $cart['count_price']; ?></em> </span>
                             ...
                             <!--</span>-->
                                    ...
                                    <a href="<?php echo U('Mobile/Goods/goodsInfo',array('id'=>$cart[goods_id])); ?>"><?php echo $cart[goods_name]; ?></a>
                                        ...
                                        <?php if($cart['goods_price']-$cart['member_goods_price'] !=0): ?>
                                            <p class="price-foot-two">共省：<i>￥</i> <em id="cart_<?php echo $cart['id']; ?>_save_price"><?php echo $cart['goods_price']-$cart['member_goods_price']; ?></em></p>
                                        <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <a href="<?php echo U('Mobile/Goods/goodsInfo',array('id'=>$cart_r[goods_id])); ?>"><?php echo $cart_r[goods_name]; ?></a>
                                            ...
                                            <div class="price-foots">
                                                <?php if($cart_r['goods_price']-$cart_r['member_goods_price'] !=0): ?>
                                                    <p class="price-foot-two">共省：<i>￥</i> <em id="cart_<?php echo $cart_r['id']; ?>_save_price"><?php echo $cart_r['goods_price']-$cart_r['member_goods_price']; ?></em></p>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                    </div>
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function(data){
                if(data.status == 1){
                    ...
                    $('#total_fee').empty().html(data.result.total_fee.toFixed(2));
                    $('#goods_fee').empty().html('节省：￥'+data.result.goods_fee.toFixed(2));
                    var cartList =  data.result.cartList;
                    if(cartList.length > 0){
                        for(var i = 0; i < cartList.length; i++){
                            $('#cart_'+cartList[i].id+'_goods_price').empty().html('￥'+Number(cartList[i]['goods_price']).toFixed(2));
                            // $('#cart_'+cartList[i].id+'_member_goods_price').empty().html('￥'+parseInt(cartList[i].member_goods_price).toFixed(2));
                            $('#cart_'+cartList[i].id+'_total_price').empty().html('￥'+cartList[i].total_fee.toFixed(2));
                            $('#cart_'+cartList[i].id+'_market_price').empty().html('￥'+cartList[i].member_goods_price*cartList[i].goods_num.toFixed(2)); //活动价格
                            $('#changeQuantity_'+cartList[i].id).val(cartList[i].goods_num); //数量
                            //共省多少钱
                            countDiscount();
                    ...
                    $('#goods_num').empty().html(data.result.goods_num);
                    $('#total_fee').empty().html(parseInt(data.result.total_fee).toFixed(2));
                    $('#goods_fee').empty().html('节省：￥'+parseInt(data.result.goods_fee).toFixed(2));
                }
            }
        });
    }
    ...
    function countDiscount(){
        $('.Set-meal-wrap ').each(function(i,o){
            ...
            var countPrice = 0;
            s.each(function () {
                var price = $(this).find('.price-foot-two em').text();
                var num = $(this).find('.mp_mp input').val();
                countPrice += Number(price * num);
            })
...
//  默认选中虚线
    ...
    function recoveryGoods(id,item) {
        $.ajax({
            type: "POST",
            url: "<?php echo U('Home/Cart/ajaxAddCart'); ?>",
